...
class RobotWrapper(pin.RobotWrapper):
    ...
    def addPointCloudAsConvex(self, points, name="point_cloud", pose=None):
        ...
        try:
            ...
            print("not adding point cloud as coplanar")
            exit()
        print("--------3---------")
        print(hull)
        ...
        print("Vertices (numpy array):", vertices)
        print("Faces (numpy array):", faces)
        ...
        convex_shape = fcl.Convex(vs, fs)
        self.addGeometry(name, convex_shape, pose)
    ...
    def attachConvexFromPointCloud(self, points, pose, frame_index, color=(1, 1, 0, 1), name="attached_convex"):
        point_cloud = o3d.geometry.PointCloud()
        point_cloud.points = o3d.utility.Vector3dVector(points)
        hull, _ = point_cloud.compute_convex_hull()
        vertices = np.asarray(hull.vertices)
        ...
        # Create the Convex object
        convex = fcl.Convex(vs, fs)
        frame = self.get_frame(frame_index)
        pose = frame.placement * toSE3(pose)
        self.addGeometry(name, convex, pose, parent_joint=frame.parent, color=color)
    ...
    def addPointCloudVisual(self, points, pose=None, name="point_cloud"):
        # Create the BVHModel for visual purposes
        visual_pcd = fcl.BVHModel()
        visual_pcd.beginModel(0, len(points))
        visual_pcd.addVertices(points)
        visual_pcd.endModel()

        # Create the GeometryObject for visualization
        visual_go = self.addGeometry(name, visual_pcd, pose, add_collision=False)
        self.visual_model.addGeometryObject(visual_go)

        # Create the BVHModel for collision purposes
        collision_pcd = fcl.BVHModel()
        collision_pcd.beginModel(0, len(points))
        collision_pcd.addVertices(points)
        collision_pcd.endModel()

        # Create the GeometryObject for collision
        collision_go = self.addGeometry(name + "_collision", collision_pcd, pose, add_collision=True)
        self.collision_model.addGeometryObject(collision_go)
    ...
    def addMeshVisual(self, vertices, faces, pose=None, name="mesh"):
        ...
        model.beginModel(len(vertices),0)
